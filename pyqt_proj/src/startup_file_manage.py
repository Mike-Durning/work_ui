...
class FileManager:
    ...
    def return_path(self):
        return_dir = askdirectory(title="Select Client Folder")
        return return_dir
    ...
    def save_client_info_to_json(self, client_info):
        try:
            with open(self.search_list_format_info, "w") as json_file:
                json.dump(client_info, json_file, indent=4)
        except Exception as e:
            print(f"An error occurred while saving client data: {str(e)}")
    ...
    def save_xlsx(self, wb, search_list_format_info):
        ...
        download_directory = search_list_format_info["custom_dir"]

        filename_save = client_name + " " + search_list + " " + todays_date + file_type  # noqa: E501

        if download_directory is None:
            download_directory = askdirectory(title="Select Client Folder", initialdir=self.clients_folder_path)
