from PyQt6.QtWidgets import QTabWidget, QMainWindow, QWidget, QVBoxLayout, QGridLayout, QLineEdit  # noqa: E501
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        for text, (row, col) in add_remove_client_position.items():
            ...
            button_layout.addWidget(self.line_edit, row, col)
            if text == "Add Client":
                pass
            if text == "Remove Client":
                pass
        ...
        self.toggle_states = {
            ...
            "Toggle Custom Directory" : False
        }

        self.file_manager.save_toggle_states_to_json(self.toggle_states)
        self.file_manager.save_client_info_to_json(self.search_list_format_info)
    ...
    def client_add(self):
        self.output_text1.append("client added")  

    def client_del(self):
        self.output_text1.append("client removed")
    ...
    def is_lhi(self):
        if self.sender().isChecked():
            self.output_text1.append("'Toggle Shortened Search List' is ON")
            self.toggle_states["Toggle Shortened Search List"] = True

        else:
            self.output_text1.append("'Toggle Shortened Search List' is OFF")
            self.toggle_states["Toggle Shortened Search List"] = False
        self.file_manager.save_toggle_states_to_json(self.toggle_states)
        self.clear_repop()
    ...
    def is_default_path(self):
        if self.sender().isChecked():
            ...
            custom_dir = self.file_manager.return_path()
            self.search_list_format_info["custom_dir"] = custom_dir
            self.toggle_states["Toggle Custom Directory"] = True
            

        else:
            self.output_text1.append("'Toggle Custom Directory is OFF")
            self.toggle_states["Toggle Custom Directory"] = False
        self.file_manager.save_toggle_states_to_json(self.toggle_states)
        self.clear_repop()
