...
class ExcelManipulator:
    def __init__(self):
        ...
        self.alphabet_path = self.file_manager.excel_manipulator_locations()[1]
        
        self.toggle_path = self.file_manager.excel_manipulator_locations()[2]

        self.alphabet_dict = self.file_manager.json_dict(self.alphabet_path)
    ...
    def openpyxl_format_workbook(self, concatenated_excel_file): 
        alphabet = open(self.alphabet_path, 'r')

        alphabet = json.load(alphabet)
        ...
        table_exists = False
        for table_name, _ in worksheet.tables.items():
            if table_name == "Table1":
                table_exists = True
                break
            
        if not table_exists:
            # Make sure the table encompasses all columns and rows
            table = Table(displayName= "Table1", ref= table_dimension)
